...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($result->num_rows > 0) {
        // Admin exists, proceed with updating member details
        // Prepare update statement
        if ($attribute === "emailMember" || $attribute === "passMember") {
            $sql = "UPDATE Member SET $attribute = ? WHERE memberID = ?";
            $stmt = $conn1->prepare($sql);
            $stmt->bind_param("si", $newValue, $memberID);
            ...
            echo "Invalid attribute specified.";
        }
    } else {
